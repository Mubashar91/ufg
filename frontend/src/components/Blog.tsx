  ...
  useEffect(() => {
    ...
    const fetchBlogs = async () => {
      try {
        setLoading(true);
        setError(null);
        const result = await apiService.getBlogs(lang);
        if (!cancelled) {
          setBlogs((result.blogs || []).slice().sort((a, b) => (a.order ?? 0) - (b.order ?? 0) || a.blogId - b.blogId));
        }
      } catch (e) {
        if (!cancelled) {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-3 sm:mb-4 text-[hsl(222,47%,11%)] dark:text-foreground">
            {t("blog.title.pre")} <span className="bg-gradient-to-r from-[hsl(var(--brand-green))] via-[hsl(var(--gold))] to-[hsl(var(--brand-green))] bg-clip-text text-transparent">{t("blog.title.highlight")}</span>
          </h2>
              ...
              >
                ...
                <div className="p-4 sm:p-5 lg:p-6 flex flex-col flex-grow">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-3 border-t border-[hsl(240,40%,92%)] dark:border-[hsl(240,30%,35%)]/50">
                    <span className="text-[10px] sm:text-xs text-[hsl(var(--brand-green))] dark:text-[hsl(var(--gold))] truncate">{t("blog.by")} {post.author}</span>
